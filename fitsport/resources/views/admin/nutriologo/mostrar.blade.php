...
<div class="w-full h-screen flex flex-col items-center justify-center overflow-auto">
    ...
    <div class="rounded-xl flex text-white items-center w-4/5 mb-4" style="background-color: rgba(53, 58, 80, 0.67); padding: 15px">
        ...
        <p id="titulo" class="ml-4 mb-0">Nutriologos</p>
        <!-- Enlace para agregar un nuevo gimnasio -->
        <a href="{{route('admNutriologo.create')}}" class="ml-auto flex items-center rounded-full border-4 border-white w-12 h-12 text-white text-2xl text-center">
            ...
            <table id="example" class="mt-2 table hover hover:border-collapse">
                ...
                @foreach ($nutriologo as $nutriologo)
                    <tr>
                        ...
                        <td class="actions-cell"><a href="{{ route('admNutriologo.editar', $nutriologo->id) }}" class="edit-button">Editar</a></td>
                        <td class="actions-cell"><a href="#" onclick="eliminar({{ $nutriologo->id }})" class="edit-button">Eliminar</a></td>
                    </tr>
                @endforeach
                </tbody>
            </table>
    ...
    function eliminar(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                window.location.href = "{{ route('admNutriologo.eliminar', ':id') }}".replace(':id', id);
            }
        });
    }

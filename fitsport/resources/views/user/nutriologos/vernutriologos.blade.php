...
@section('contenido')
    <div style="font-family: 'Poppins';" class=" w-full h-screen flex  flex-col items-center justify-center overflow-auto ">
        <div class="rounded-xl flex text-white items-center w-4/5 mb-4" style="background-color:rgba(53, 58, 80, 0.67); padding: 15px">
            ...
            <p id="titulo" class="ml-4 mb-0">Nutriólogos</p>
        </div>
        <br>
        <div class="flex w-4/5 justify-end"> 
            
            <div class="rounded-xl flex justify-end items-center w-1/4 mb-4" style="padding: 15px">
                <div class="bg-white p-2 rounded-xl w-full flex items-center justify-center">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M21 21l-5.197-5.197m0 0A7.5 7.5 0 105.196 5.196a7.5 7.5 0 0010.607 10.607z" />
                    </svg>
                    <input type="text" placeholder="Buscar nutriologo" class="buscar flex-grow p-2 rounded-xl" id="searchInput">
                </div>
            </div>
        ...
        <div class="rounded-xl text-white w-4/5 mb-8">
            <div id="cardsContainer" class="flex flex-wrap"> 
                @if (count($nutriologo) > 0)
                    @foreach ($nutriologo as $index => $data)
                        <div class="card text-white mb-8">
                            <div class="flex flex-col items-center">
                                <p class="text-lg font-bold">Nutriologo </p>
                                <p class="text-lg font-bold">{{$data->nombre}} {{$data->apellido}}</p>
                                <div class="circle-container" >
                                    @if($data->fotografia)
                                        <img class="mx-0 my-0" src="{{ asset('ImgNutriologo/' . $data->fotografia) }}" alt="Foto de perfil">
                                    @else
                                ...
                                @endif --}}
                                <a href="{{route('nutriologos.verDatosNutriologo',$data->id)}}" class="mt-2">
                ...
                <div class="rounded-xl flex text-white items-center w-4/5 mb-4" style="background-color:rgba(53, 58, 80, 0.67); padding: 15px">
                    <img src="{{asset('img/cuadro.png')}}" alt="Imagen pequeña" class="h-8 w-8">
                    <p id="titulo" class="ml-4 mb-0">Sin nutriologos</p>
                </div>
...
document.getElementById('searchInput').addEventListener('input', function(e) {
    ...
    axios.get('/Nutriologos/buscando', {
        params: {
            query: query
        }
    })
    .then(function(response) {
        ...
        response.data.forEach(data => {
            let imgSrc = data.fotografia ? `ImgNutriologo/${data.fotografia}` : 'img/user.png';
            ...
            <div class="card text-white mb-8">
                <div class="flex flex-col items-center">
                    ...
                    <p class="text-lg font-bold">${data.nombre} ${data.apellido}</p>
                    <div class="circle-container" >
                        <img src="${imgSrc}" alt="Imagen del Nutriologo" >
                    </div>
